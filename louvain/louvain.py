...
class Neo4jDB:
    ...
    @staticmethod
    def load_node(tx, edge_id):
        edge_id = str(edge_id)
        node_info = tx.run("MATCH (a:louvain)-[r:edge]->(b:louvain) "
                           "WHERE r.id=$edge_id "
                           "RETURN r.s,r.e,r.weight"
                           , edge_id=edge_id)
        record = node_info.single()
        # print("node_info", type(record), record)
        x, y, z = record
        #print(x,y,z)
        return record

    def node_info(self, edge_id):
        with self.driver.session() as session:
            node = session.write_transaction(self.load_node, edge_id)
            return node
...
if __name__ == "__main__":
    greeter = Neo4jDB("bolt://localhost:7687", "johnsnow", "cyt1999127")
    G = collections.defaultdict(dict)
    for i in range(1, 6):
        vertices = greeter.node_info(i)
        v_i = int(vertices[0])
        v_j = int(vertices[1])
        w = float(vertices[2])
        G[v_i][v_j] = w
